...
- 数据保存期限：插件自身**不存储任何数据**，每次请求实时读取 Halo 评论扩展并即时返回；卸载插件即完全停止聚合。数据的保留与删除始终跟随站点评论本身。
- 兼容：Halo `>=2.25.0`；浏览器端零依赖、零构建。

## 后台设置

Console → 插件 → 最近评论 → 设置（保存后立即生效，无需重启）：

| 设置项 | 范围 | 默认 | 说明 |
| --- | --- | --- | --- |
| 默认返回条数 | 1-50 | 5 | 请求未携带 `size` 参数（或非法）时返回的条数 |
| 单次请求上限 | 1-100 | 20 | `size` 超过该值时按该值截断，防止大查询拖垮站点 |
| 结果缓存秒数 | 0-3600 | 0（禁用） | 聚合结果服务端缓存；流量较大的站点建议 30-300 秒 |

> 前端 `<recent-comments>` 组件的条数由 `size` 属性控制（见下方接入示例），后台「单次请求上限」对它同样生效。
